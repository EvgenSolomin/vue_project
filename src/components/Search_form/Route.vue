<template>
  <div class="grid justify-content-center align-items-center w-full">
    <Dropdown v-model="selectedPointA" @change="updateselectedPointA($event)" :options="pointA" filter optionLabel="name" placeholder="Откуда едем?" class="w-full md:w-14rem">
        ...
        <template #option="slotProps">
            <div class="flex align-items-center">
                <div><h5>{{ slotProps.option.name }}</h5>{{ slotProps.option.address }}</div>
            </div>
        </template>
    </Dropdown>
    ...
    <Button
      ...
      icon="pi pi-arrows-h"
    />
    <Dropdown v-model="selectedPointB" @change="updateselectedPointB($event)" :options="pointB" filter optionLabel="name" placeholder="Куда едем?" class="w-full md:w-14rem">
        <template #value="slotProps">
            <div v-if="slotProps.value" class="flex align-items-center">
              <div>{{ slotProps.value.name }}</div>
            </div>
    ...
    <!-- <InputText class="col-5 m-1" v-model="pointB" type="text" placeholder="Куда" /> -->
  </div>
</template>
